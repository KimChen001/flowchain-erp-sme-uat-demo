...
import { BarChart3, BookmarkPlus, ChevronDown, ChevronUp, Download, Info, RefreshCw, RotateCcw, SlidersHorizontal } from "lucide-react";
...
export function BiDashboard({ view, onNavigate: _onNavigate }: { view: DashboardView; onNavigate?: NavigateFn }) {
  ...
  return <div className="space-y-3" data-testid="bi-dashboard" data-view={view}>
    ...
    <Card className="flex flex-wrap items-end gap-3 p-3" data-testid="dashboard-configuration"><label className="text-[11px] font-medium" style={{ color: A.gray1 }}>比较方式<select aria-label="比较方式" value={params.get("comparison") || "previous_period"} onChange={(event) => setFilter("comparison", event.target.value, "previous_period")} className="ml-2 h-8 rounded-lg px-2" style={{ background: A.gray6 }}><option value="none">不比较</option><option value="previous_period">上期</option><option value="year_over_year">同比</option></select></label><label className="text-[11px] font-medium" style={{ color: A.gray1 }}>排名数量<select aria-label="排名数量" value={params.get("topN") || "8"} onChange={(event) => setFilter("topN", event.target.value, "8")} className="ml-2 h-8 rounded-lg px-2" style={{ background: A.gray6 }}><option value="5">Top 5</option><option value="8">Top 8</option></select></label><span className="ml-auto text-[11px]" style={{ color: A.sub }}>筛选、比较与布局均可保存为我的视图</span></Card>
    {report && <Card className="flex flex-wrap items-center gap-2 p-3" data-testid="chart-configuration"><span className="mr-1 text-[11px] font-semibold" style={{ color: A.gray1 }}>图表显示与顺序</span>{orderedCharts.map((chart, index) => <div key={chart.id} className="inline-flex items-center rounded-lg" style={{ background: A.gray6 }}><button type="button" onClick={() => toggleChart(chart.id)} className="px-2 py-1.5 text-[11px]" style={{ color: hiddenChartIds.includes(chart.id) ? A.gray2 : A.blue }}>{hiddenChartIds.includes(chart.id) ? `显示 ${chart.title}` : chart.title}</button><button type="button" aria-label={`上移 ${chart.title}`} disabled={index === 0} onClick={() => moveChart(chart.id, -1)} className="px-1 disabled:opacity-30"><ChevronUp size={11} /></button><button type="button" aria-label={`下移 ${chart.title}`} disabled={index === orderedCharts.length - 1} onClick={() => moveChart(chart.id, 1)} className="px-1 disabled:opacity-30"><ChevronDown size={11} /></button></div>)}</Card>}
