...
import { fetchTodayCockpit, type TodayCockpitAction, type TodayCockpitCard, type TodayCockpitDocument, type TodayCockpitInventoryRisk, type TodayCockpitResponse, type TodayCockpitSeverity } from "./todayCockpit";
...
function TodayCockpitV2Panel({
  ...
  if (!cockpit && loading) {
    return (
      <Card className="p-5">
        <div className="text-[13px] font-semibold" style={{ color: A.label }}>今日驾驶舱正在加载</div>
        <div className="mt-1 text-[12px]" style={{ color: A.sub }}>正在读取采购、库存和单据证据。</div>
      </Card>
    );
  }
  ...
  const followups = cockpit.followups.slice(0, 4);
  ...
  return (
    <Card className="p-5">
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-2">
        <CockpitActionList title="优先跟进" items={followups.map((item) => ({
          id: item.id,
        ...
        <CockpitInventoryRiskList items={inventoryRisks} onNavigate={onNavigate} />
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-5">
        ...
        <CockpitActionList title="建议动作" items={actions} onNavigate={onNavigate} className="xl:col-span-2" />
      </div>
    </Card>
  );
}

function CockpitActionList({
  title,
  items,
  onNavigate,
  className = "",
...
function CockpitInventoryRiskList({
  items,
  onNavigate,
}: {
...
export default function OverviewPanel({ onNavigate, onPrepareReplenishmentRequest, onOpenAi }: OverviewPanelProps) {
  ...
  useEffect(() => {
    let alive = true;
    fetchTodayCockpit()
      .then((data) => { if (alive) setTodayCockpit(data); })
      .catch(() => { if (alive) setTodayCockpit(null); })
  ...
  return (
    <div className="space-y-4">
      <TodayCockpitV2Panel cockpit={todayCockpit} loading={todayCockpitLoading} onNavigate={onNavigate} />
