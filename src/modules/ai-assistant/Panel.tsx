  ...
  useEffect(() => {
    ...
    apiJson<{ asOf: string; prices: MarketPrice[] }>("/api/market-prices")
      .then((data) => {
        ...
        setMarketStatus(`行情数据 ${data.prices.length} 条`);
      })
      .catch(() => setMarketStatus("行情数据暂不可用"));
    return () => { if (timer.current) clearTimeout(timer.current); };
