...
export default function ContextualImportActions({ entityLabel, templateName, compact = false }: ContextualImportActionsProps) {
  ...
  async function downloadTemplate() { const filename = await downloadExcelTemplate(schema); toast.success("模板已下载", { description: filename }); }
  ...
  return <>
    <input ref={inputRef} type="file" className="hidden" data-testid="excel-import-file-input" accept=".xlsx,.xls,.csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel,text/csv" onChange={(event) => { selectFile(event.target.files?.[0]); event.currentTarget.value = ""; }} />
