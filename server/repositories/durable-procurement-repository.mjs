...
const empty = () => ({ schemaVersion:1, purchaseRequests:[], rfqs:[], purchaseOrders:[], idempotencyRecords:[], auditEntries:[], updatedAt:new Date().toISOString() })
...
export function createDurableProcurementRepository({ dataFile, seed = empty() }) {
  let document
  async function load() { if (document) return document; try { document=JSON.parse(await readFile(dataFile,'utf8')) } catch (e) { if(e.code!=='ENOENT') throw e; document=clone(seed); document.schemaVersion=1; await atomicWrite(dataFile,document) } return document }
