...
export function createDurableProcurementRepository({ dataFile, seed = emptyProcurementRuntime() }) {
  let document
  async function readLatest() { let latest; try { latest=JSON.parse(await readFile(dataFile,'utf8')) } catch (e) { if(e.code!=='ENOENT') throw e; latest=clone(seed) }
    latest = { ...emptyProcurementRuntime(), ...latest, initialized: true }
    for (const key of ['purchaseRequests','rfqs','purchaseOrders','receipts','supplierInvoices','matchRecords','purchaseReturns','workItems','auditEvents','auditEntries','idempotencyRecords']) if (!Array.isArray(latest[key])) latest[key]=[]
    return latest }
  ...
  return {
    ...
    async transact(fn){ return withRuntimeFileMutex(dataFile, async () => { const working=clone(await readLatest()); const result=await fn(working); working.revision=Number(working.revision||0)+1; working.updatedAt=new Date().toISOString(); await atomicWrite(dataFile,working); document=working; return clone(result) }) },
