...
export function createReportView(input, actor) {
  ...
  const view = { viewId, name: text(input.name), description: text(input.description), ownerId: actor.id, ownerName: actor.name, subject: input.subject, sourceRoute: input.sourceRoute || '/app/reports/library', columns: clone(input.columns || []), columnOrder: clone(input.columnOrder || input.columns || []), filters: clone(input.filters || {}), sorting: clone(input.sorting || []), grouping: clone(input.grouping || []), measures: clone(input.measures || []), visualization: input.visualization || 'table', comparison: input.comparison || 'previous_period', hiddenCharts: clone(input.hiddenCharts || []), chartOrder: clone(input.chartOrder || []), topN: [5, 8].includes(Number(input.topN)) ? Number(input.topN) : 8, dateRange: clone(input.dateRange || null), visibility: input.visibility || 'private', isDefault: Boolean(input.isDefault), createdAt: now, updatedAt: now, lastOpenedAt: now, version: 1 }
