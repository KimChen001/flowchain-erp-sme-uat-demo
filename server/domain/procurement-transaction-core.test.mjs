...
test('R270 guardrails preserve confirmed action boundary provider-free UI and R250 cleanup', async () => {
  ...
  const prPage = readSource('src', 'modules', 'purchase-requests', 'CanonicalProcurementPanel.tsx')
  const rfqPage = readSource('src', 'modules', 'procurement', 'Page.tsx')
  const allSource = [
  ...
  assert.match(prPage, /\/api\/procurement\/requests/)
  assert.match(prPage, /generate-purchase-orders/)
  assert.match(prPage, /生成 Draft PO/)
  assert.doesNotMatch(prPage, /\/api\/rfqs",\s*\{\s*method:\s*"POST"/)
  assert.match(rfqPage, /暂无询价单/)
  assert.doesNotMatch(rfqPage, /RFQS|demo-data/)
