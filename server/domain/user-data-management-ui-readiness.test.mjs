...
test('R181-R190 source guardrails keep UI review-first and provider-free', () => {
  ...
  assert.match(importsPage, /Review-first commit/)
  assert.match(importsPage, /AI remains read-only/)
  assert.match(importsPage, /cannot submit, approve, pay, post, send, or mutate business records/)
  assert.match(importsPage, /writesFiles: \$\{String\(userPreview\?\.writesFiles/)
  assert.match(importsPage, /overwritesDemoData: \$\{String\(userPreview\?\.overwritesDemoData/)
