...
test('health reports authoritative mode/adapters and local login is independent of demo db', async () => {
  ...
  try {
    ...
    assert.equal('worktree' in health.payload, false)
    assert.equal(typeof health.payload.runtimeMode, 'string')
    ...
    const serializedHealth = JSON.stringify(health.payload)
    assert.doesNotMatch(serializedHealth, /[A-Za-z]:\\|\/Users\/|\/home\//)
    assert.doesNotMatch(serializedHealth, /worktree|cwd|provider|model|proxy|secret|token|password|api[_-]?key/i)
