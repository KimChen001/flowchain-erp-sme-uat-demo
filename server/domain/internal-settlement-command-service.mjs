...
const authorize = (actor, permission) => {
  ...
  if (["finance.cashbook.manage", "finance.settlement.create", "finance.settlement.post", "finance.settlement.reverse"].includes(permission))
...
async function settlementPlan(db, tenantId, input) {
  ...
  for (const row of input.allocations) {
    ...
    try { total += amountUnits(row.amount, "Allocation amount"); } catch (error) { add(error.code, error.message, error.status); }
  }
  let requested = 0n;
  try { requested = amountUnits(input.amount, "Settlement amount"); } catch {}
  if (requested && total !== requested) add("SETTLEMENT_ALLOCATION_TOTAL_MISMATCH", "Allocation total must equal the settlement amount.", 422, { settlementAmount: fixed(requested), allocationTotal: fixed(total) });
  ...
  return {
    ...
    account,
    obligations,
    normalized: { ...input, amount: requested ? fixed(requested) : "0.0000", counterpartyId: input.counterpartyId || first?.counterpartyId || "", counterpartyNameSnapshot: first?.counterpartyName || "" },
...
export function createInternalSettlementCommandService({ prisma, env = process.env, idFactory = randomUUID, now = () => new Date() } = {}) {
  ...
  async function createSettlement(input, context) {
    ...
    return execute("create_settlement", input, context, payload, async (tx, actor, command) => {
      ...
      const settlement = await tx.settlementDocument.create({ data: {
        id: idFactory(), tenantId: actor.tenantId, settlementNumber: plan.normalized.settlementNumber, direction: plan.normalized.direction, counterpartyType: plan.normalized.counterpartyType, counterpartyId: plan.normalized.counterpartyId || null, counterpartyNameSnapshot: plan.normalized.counterpartyNameSnapshot || null, cashbookAccountId: plan.normalized.cashbookAccountId, currency: plan.normalized.currency, amount: plan.normalized.amount, settlementDate: asDate(plan.normalized.settlementDate, "settlementDate"), externalReference: plan.normalized.externalReference || null, memo: plan.normalized.memo || null, metadata: { externalReferenceVerified: false, bankExecution: false, ledgerMutation: false },
        allocations: { create: plan.normalized.allocations.map((row) => ({ id: idFactory(), tenantId: actor.tenantId, obligationType: row.obligationType, payableObligationId: row.obligationType === "payable" ? row.obligationId : null, receivableObligationId: row.obligationType === "receivable" ? row.obligationId : null, amount: fixed(units(row.amount)), currency: plan.normalized.currency })) },
  ...
  async function postSettlement(settlementId, input, context) {
    ...
    return execute("post_settlement", input, context, payload, async (tx, actor, command) => {
      ...
      if (settlement.status !== "draft") fail("SETTLEMENT_NOT_POSTABLE", "Only a draft settlement may be posted.", 409);
      ...
      const planInput = normalizeSettlement({ ...settlement, amount: fixed(units(settlement.amount)), settlementDate: settlement.settlementDate.toISOString(), allocations: settlement.allocations.map((row) => ({ obligationType: row.obligationType, obligationId: row.payableObligationId || row.receivableObligationId, amount: fixed(units(row.amount)) })) });
      ...
      const posted = await tx.settlementDocument.update({ where: { id: settlement.id }, data: { status: "posted", postedAt: now(), postedById: actor.user.id, version: { increment: 1 } } });
      ...
      return { entityType: "SettlementDocument", entityId: settlement.id, settlement: { id: posted.id, settlementNumber: posted.settlementNumber, status: posted.status, version: posted.version }, cashbookEntry: { id: entry.id, entryNumber: entry.entryNumber, direction: entry.direction, amount: fixed(amount), currency: entry.currency, balanceBefore: fixed(before), balanceAfter: fixed(after) } };
    });
  }
  ...
  async function reverseSettlement(settlementId, input, context) {
    ...
    return execute("reverse_settlement", input, context, payload, async (tx, actor, command) => {
      ...
      const reversed = await tx.settlementDocument.update({ where: { id: settlement.id }, data: { status: "reversed", reversedAt: now(), reversedById: actor.user.id, reversalReason: payload.reason, version: { increment: 1 } } });
  ...
  return { previewSettlement, createCashbookAccount, createSettlement, postSettlement, reverseSettlement };
}
