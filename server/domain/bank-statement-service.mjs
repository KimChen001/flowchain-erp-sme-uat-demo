...
export function createBankStatementService({ prisma, env = process.env, storageProvider, idFactory = randomUUID, now = () => new Date() } = {}) {
  ...
  async function updateMapping(id, input, context) {
    ...
    try { assertSafeBankMappingConfiguration({ columnMapping: input.columnMapping || current.columnMapping, metadata: input.metadata }); } catch (error) { fail(error.code, error.message, error.status, error.details); }
