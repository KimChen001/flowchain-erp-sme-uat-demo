...
test("settlement workflow, advance application, internal transfer, and sync feed remain atomic", { skip: !enabled }, async () => {
  ...
  try {
    ...
    await assert.rejects(() => sync.changes({ clientId: registered.clientId, deviceId: "phase52b-device-a", cursor: `${initial.cursor.slice(0, -1)}x` }, admin), (error) => error.code === "SYNC_CURSOR_TAMPERED");
