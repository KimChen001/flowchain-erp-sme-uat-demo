...
test('Phase 0 product positioning and visible language governance stay productized', () => {
  ...
  const limitations = readSource('docs', 'uat-limitations-v1.md')
  ...
  const publicDocs = [readme, docsIndex, narrative, roadmap, limitations, language].join('\n')
  const uiCopySources = [constants, forecast, actionShell, overview].join('\n')
  ...
  assert.match(forecast, /当前物料需求计划使用样例商品、静态计划参数/)
  assert.match(actionShell, /保存待复核草稿/)
  assert.match(overview, /库存周转/)

  assert.doesNotMatch(readme.split('\n')[0], /UAT|Demo|ERP\/SCM/)
  assert.doesNotMatch(publicDocs, /FlowChain ERP\/SCM UAT Demo|JSON\/demo-data-backed UAT|UAT limitations/)
  ...
  assert.doesNotMatch(uiCopySources, /ActionDraft purchase_request_draft|provider fallback|tool_result|response_card|raw JSON/)
})
