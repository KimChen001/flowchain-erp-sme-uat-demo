...
export function createAttachmentService({ prisma, env = process.env, idFactory = randomUUID, now = () => new Date() } = {}) {
  if (!prisma) throw new Error("prisma is required");
  const root = resolve(text(env.FLOWCHAIN_UPLOAD_STORAGE_DIR) || join(tmpdir(), "flowchain-staged-uploads"));
  ...
  async function cleanupExpiredUploads() {
    ...
    for (const upload of expired) {
      await unlink(join(root, upload.tenantId, upload.id)).catch(() => {});
  ...
  async function stageUpload(input, context) {
    ...
    const directory = join(root, actor.tenantId); await mkdir(directory, { recursive: true }); await writeFile(join(directory, id), bytes, { flag: "wx" });
    const upload = await prisma.stagedUpload.create({ data: { id, tenantId: actor.tenantId, fileName, mimeType, sizeBytes: bytes.length, sha256, storageKey, createdById: actor.user.id, expiresAt: new Date(now().getTime() + 24 * 60 * 60 * 1000), metadata: { binaryInBusinessJson: false } } });
  ...
  async function download(attachmentId, context) {
    ...
    const filePath = join(root, attachment.tenantId, attachment.upload.id), bytes = await readFile(filePath);
  ...
  return { stageUpload, status, bindSettlement, bindReceiving, download, deleteAttachment, cleanupExpiredUploads };
}
