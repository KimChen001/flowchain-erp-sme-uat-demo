...
function runtimeRows(context, inventory) {
  return {
    ...
    supplier_invoices: array(context.supplierInvoices).map(row => ({ id: text(row.id || row.invoiceNumber), date: date(row), supplier: text(row.supplierName || row.supplierId), amount: number(row.totalAmount ?? row.amount), status: text(row.status), currency: text(row.currency || 'CNY') })),
...
function filtered(rows, query) {
  return rows.filter(row => (!query.from || !row.date || row.date >= query.from) && (!query.to || !row.date || row.date <= query.to) && (!query.supplier || row.supplier === query.supplier) && (!query.customer || row.customer === query.customer) && (!query.currency || row.currency === query.currency) && (!query.status || row.status === query.status))
}
