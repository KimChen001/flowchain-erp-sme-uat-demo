...
test("real PostgreSQL bank statement and reconciliation authority", async (t) => {
  ...
  try {
    ...
    await t.test("deterministic candidates and one-to-one confirmation preserve Cashbook", async () => {
      ...
      const first = await reconciliation.generateCandidates(target.id, {}, context), second = await reconciliation.generateCandidates(target.id, {}, context); assert.deepEqual(first.candidates.map((item) => [item.cashbookEntryId, item.score, item.algorithmVersion]), second.candidates.map((item) => [item.cashbookEntryId, item.score, item.algorithmVersion])); assert.equal(first.algorithmVersion, "bank-match-v1.1"); assert.equal(first.candidates[0].evidence.recommendation, "recommended"); assert.equal(first.candidates[0].cashbookConfirmedAllocatedAmount, "0.0000"); assert.equal(first.candidates[0].cashbookRemainingAmount, first.candidates[0].cashbookOriginalAmount);
    ...
    await t.test("partial allocations, over-allocation and integrity exceptions are controlled", async () => {
      ...
      const partialCandidates = await reconciliation.generateCandidates(target.id, {}, context), partial = partialCandidates.candidates.find((item) => item.cashbookEntryId === "cashbook-bank-2"); assert.ok(partial); assert.equal(partial.cashbookConfirmedAllocatedAmount, half); assert.equal(partial.cashbookRemainingAmount, (await statement.getLine(target.id, context)).remainingAmount);
      ...
      await reconciliation.refreshIntegrity(context);
      ...
      const exception = exceptions.items.find((item) => item.exceptionType === "cashbook_entry_reversed"); await assert.rejects(() => reconciliation.resolveException(exception.id, { resolutionType: "remediated", resolution: "Not actually fixed", evidence: { reviewId: "REVIEW-1" } }, context), (error) => error.code === "BANK_RECONCILIATION_INTEGRITY_NOT_REMEDIATED"); const acknowledged = await reconciliation.resolveException(exception.id, { resolutionType: "acknowledged", resolution: "Known unresolved reversal", evidence: { reviewId: "REVIEW-2" } }, context); assert.equal(acknowledged.status, "acknowledged");
